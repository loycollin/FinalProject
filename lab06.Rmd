...
```{r}
athletes = read_csv("https://raw.githubusercontent.com/loycollin/FinalProject/master/athlete_events.csv")
regions = read_csv("https://raw.githubusercontent.com/loycollin/FinalProject/master/noc_regions.csv")

str(athletes)

athletes %>% ggplot(aes(x = Sex)) + geom_bar()
athletes %>% ggplot(aes(x = Age)) + geom_bar()
athletes %>% ggplot(aes(x = Height)) + geom_bar()
athletes %>% ggplot(aes(Weight)) + geom_histogram()
athletes %>% ggplot(aes(x = Weight)) + geom_bar()
athletes %>% ggplot(aes(x = Sport)) + geom_bar() + coord_flip()
...
In Canvas, submit an RMarkdown file (and the knitted html document) with: 
  ...
  b. an overview of the data set you are working with for the final project
      ...
      + We plan to compare results for the games for different years of the data, and look into the atheletes listed for things like how any medals were won by each.
      + This will also include looking at how countries stack up against one another.
    
  c. a discussion of cleaning steps that are necessary to get the data into the right shape
      + We will need to convert some of the variables to factors, and change the Year variable to time. We can also join the regions to the athletes table. 

---
title: "Final Project: The Olympics"
...
# List of variables and their formats.
str(olympics)

# 5 number summaries of each variable. 
summary(olympics)

## Data Exploration: Summarize Variables

# Gender: Created a bar chart and filled by sex. There are over twice as many male athletes.
...
# Age: For age, we summarized the average age by region, then created a histogram by region. We also created a histogram of all athelete ages. The plot is right-skewed, with 23 being the most common age. 
olympics %>% ggplot(aes(x = Age)) + geom_bar(fill = "navy")
olympics %>% group_by(region) %>% summarise(mean_age = mean(Age, na.rm=TRUE)) 
olympics %>% group_by(region) %>% summarise(mean_age = mean(Age, na.rm=TRUE)) %>% ggplot(aes(mean_age)) + geom_histogram(binwidth = 0.25, fill = "navy") + xlab("Age") + ylab("Count") + ggtitle("Mean Olympic Athlete Age")


# Height: For height, we summarized the average height by region, and created a histogram of all athlete heights. The plot shows a normal distribution, with the mean height at 175 cm. 
...
# Weight: We did the same summarization for weight as height. This plot is right skewed, with the average olympic weight at around 70 kg.
...
# Regions: Created a summary of the number of the athletes from each region
olympics %>% group_by(region) %>% count() 
names = olympics %>% group_by(region, Name) %>% select(Name, region) 

# Medals: Looking at the number of medals by region.
olympics %>% group_by(region, Medal) %>% count() %>% ggplot(aes(n)) + geom_histogram()
```                                                                       

+ All of the visual summaries for each variable match expectations. The variability in average age over time was somewhat surprising. 
...
# Scatterplots for two numeric variables: numeric vs numeric
  ...
  # Athlete Height vs Weight: a scatterplot of all athletes' weight vs height. The result was expected- as height increases, so does weight. 
olympics %>% ggplot(aes(x = Height, y = Weight)) + geom_jitter(colour = "navy", size = 0.5) + ylab("Weight (kg)") + xlab("Height (cm)") + ggtitle("Athlete Height vs Weight")
 ...
 # Athlete Age vs Height: there doesn't appear to be much of a correlation between age and height.
olympics %>% ggplot(aes(x = Age, y = Height)) + geom_jitter(colour = "navy", size = 0.1) + ylab("Height (cm)") + ggtitle("Athlete Age vs Height")

  # Average Age Over Time: height increased with time, then took a dip in 1980, and has been trending upward in recent years. We added a smooth line to highlight the wave pattern.
olympics %>% group_by(Year) %>% summarise(mean_age = mean(Age, na.rm=TRUE)) %>% ggplot(aes(x = Year, y = mean_age)) + geom_line(color = "navy") + ylab("Mean Age") + ggtitle("Mean Athlete Age Over Time") + geom_smooth()
  ...
  # Athlete Age vs Weight: similar to age vs height, there doesn't appear to be a strong correlation.
olympics %>% ggplot(aes(x = Age, y = Weight)) + geom_jitter(colour = "navy", size = 0.1) + ylab("Weight (kg)") + ggtitle("Athlete Age vs Weight")

# Side-by-side boxplots or facetted plots for numeric/categorical  

  # Number of athletes, grouped by Sex, and facetted by Summer/Winter games. The total number of athletes has been trending upward. There are several dips in the male group, which may be caused by major world wars. There have never been more female athletes, but the gap between men vs women appears to be shrinking.
olympics %>%  group_by(Year, Sex, Season) %>% count() %>% ggplot(aes(x = Year, y = n, colour = Sex)) + geom_point() + geom_line() + ggtitle("Number of Athletes: Summer vs Winter Olympics") + ylab("Count") + facet_wrap(~Season)

    # Female vs Male athletes by year for summer olympics, a closer look: 
olympics %>% filter(Season == "Summer") %>%  group_by(Year, Sex) %>% count() %>% ggplot(aes(x = Year, y = n, colour = Sex)) + geom_point() + geom_line() + ggtitle("Number of Athletes: Summer Olympics") + ylab("Count")
  ...
  # Female vs Male athletes by year for winter olympics, a closer look:
olympics %>% filter(Season == "Winter") %>%  group_by(Year, Sex) %>% count() %>% ggplot(aes(x = Year, y = n, colour = Sex)) + geom_point() + geom_line() + ggtitle("Number of Athletes: Winter Olympics") + ylab("Count")
  ...
  # Age vs Number of Medals: We filtered out only the athletes that receieved a medal, grouped them by age and medal, and then calculated the number of medals   received for each age. The graphs have a similar distribution to the age histogram, which is likely the effect from having more participants in the similar    # shape. 
...
# Facet maps of gold, silver, and bronze medals in the world:

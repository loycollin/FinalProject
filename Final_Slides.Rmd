...
```{r echo=FALSE, message=FALSE, warning=FALSE}
olympics %>% group_by(Year) %>% summarise(mean_age = mean(Age, na.rm=TRUE)) %>% ggplot(aes(x = Year, y = mean_age)) + geom_line(color = "navy") + ylab("Mean Age") + ggtitle("Mean Athlete Age Over Time") 
```
